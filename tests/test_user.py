...
def test_find_missing_pieces(sample_user):
    ...
    missing = sample_user.find_missing_pieces(required)
...
def test_find_collaborators():
    ...
    # Main user missing 2 blue and 2 red
    user_main = User(1, "builderA", 4, {
        Piece("3023", "blue"): 3,
        Piece("4286", "red"): 1,
    })

    # helper1 can cover blue, helper2 can cover red
    user1 = User(2, "helper1", 5, {
        Piece("3023", "blue"): 5
    })
    user2 = User(3, "helper2", 5, {
        Piece("4286", "red"): 3
    })
    user3 = User(4, "random", 3, {
        Piece("3001", "green"): 2
    })  # irrelevant

    collaborators = user_main.find_collaborators(
        target_set,
        [user1, user2, user3, user_main]
    )
